...
export function handleFirestoreError(error: unknown, operationType: OperationType, path: string | null) {
  ...
  const errInfo: FirestoreErrorInfo = {
    error: isOffline ? `${errorMessage} (Tip: Check your network connection or Firebase Project settings if this persists)` : errorMessage,
    authInfo: {
...
async function testConnection() {
  try {
    ...
    // Connection test failed, likely harmless in dev
  }
}
testConnection();
